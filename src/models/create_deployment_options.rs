...
// &CreateDeploymentOptions: Into<CreateDeploymentOptions>
...
impl From<&CreateDeploymentOptions> for ContainerCreateBody {
    fn from(deployment_options: &CreateDeploymentOptions) -> Self {
        ...
        // Set up Host Config
        let mut port_bindings_map = None;
        let mut volume_binding_map = None;
        if let Some(mongodb_port) = &deployment_options.mongodb_port_binding {
            let port_bindings = mongodb_port.into();
            port_bindings_map = Some(hashmap! {"27017/tcp".to_string() => Some(port_bindings)});

            create_container_config.exposed_ports = Some(hashmap! {
                "27017/tcp".to_string() => hashmap! {},
            });
        }

        if let Some(local_seed_location) = &deployment_options.local_seed_location {
            volume_binding_map = Some(vec![format!(
                "{}:{}:rw",
        ...
        let host_config = HostConfig {
            ...
            binds: volume_binding_map,
            ..Default::default()
        };
        if host_config.port_bindings.is_some() || host_config.binds.is_some() {
...
mod tests {
    ...
    fn test_into_container_create_body_minimal() {
        ...
        assert!(container_create_body.host_config.is_none());
        assert_eq!(
            container_create_body
                .labels
                .unwrap()
    ...
    fn test_into_create_container_options_minimal() {
        ...
        assert_eq!(
            ...
            Some(create_deployment_options.name)
        );
    }
